...
try {
        ...
        COALESCE((
            SELECT SUM(
                CASE 
                    ...
                    ELSE 0
                END
            ) FROM descobertas d JOIN adesivos a ON d.adesivo_id = a.id WHERE d.descobridor_id = u.id AND d.is_latest = 1
        ), 0) + 
    ...
    $titulo = 'Novato';
    if ($xp >= 500) $titulo = 'Lenda do Bando';
    elseif ($xp >= 200) $titulo = 'Caçador Experiente';
    elseif ($xp >= 50) $titulo = 'Explorador';

    // ==========================================
    // 2. BUSCAR TODO O HISTÓRICO PARA AS MEDALHAS
    // ==========================================
    
    // Adesivos colados por ele
    $stmtCriados = $pdo->prepare("SELECT COUNT(*) FROM adesivos WHERE criador_id = ?");
    $stmtCriados->execute([$alvo_id]);
    $total_criados = (int)$stmtCriados->fetchColumn();

    // Missões cumpridas
    $stmtMissoes = $pdo->prepare("SELECT COUNT(*) FROM missoes_concluidas WHERE usuario_id = ?");
    $stmtMissoes->execute([$alvo_id]);
    $total_missoes = (int)$stmtMissoes->fetchColumn();

    // Histórico de descobertas (para varrer no PHP)
    $stmtDesc = $pdo->prepare("
        SELECT d.tipo_registro, d.data_descoberta, d.comentario, a.categoria, a.raridade 
        FROM descobertas d JOIN adesivos a ON d.adesivo_id = a.id 
        WHERE d.descobridor_id = ?
    ");
    $stmtDesc->execute([$alvo_id]);
    ...
    $c_raro = 0; $c_lendario = 0;
    $c_comentarios = 0;

    // Loop super rápido varrendo as descobertas
    foreach ($descobertas as $d) {
        ...
        $dia_sem = (int)date('w', strtotime($d['data_descoberta'])); // 0 (Dom) a 6 (Sáb)

        // Conta Categorias
        if ($cat === 'Praia') $c_praia++;
        if ($cat === 'Turísticos') $c_turistico++;
        if ($cat === 'Móveis') $c_moveis++;
        if ($cat === 'Estados' || $cat === 'Internacionais') $c_estados_int++;
        if ($cat === 'Estrada') $c_estrada++;

        // Conta Raridades
        if ($rar === 'Raro') $c_raro++;
        if ($rar === 'Lendário') $c_lendario++;

        // Conta Tipos de Registro
        if ($tipo === 'conquistado') $c_conquistado++;
        if ($tipo === 'encontrado') $c_encontrado++;

        // Conta Comentários
        if (!empty($coment)) $c_comentarios++;

        // Conta Tempo (Horários e Dias)
        if ($hora >= 0 && $hora < 5) $c_coruja++; // 00h às 04h59
        if ($hora >= 5 && $hora <= 9) $c_galo++; // 05h às 09h59
        if ($dia_sem === 0 || $dia_sem === 6) $c_rolezeiro++; // Sábado ou Domingo
    }

    // ==========================================
    // 3. ATRIBUIR MEDALHAS LOGICAMENTE
    // ==========================================
    $medalhas = [
        // Originais
        ['nome' => 'Pioneiro', 'icone' => '🌟', 'desc' => 'Caçador da primeira geração do Mapa dos Bandesivos.', 'desbloqueada' => true],
        ...
        ['nome' => 'Decorador', 'icone' => '🚗', 'desc' => 'Achou um adesivo de Móveis.', 'desbloqueada' => $c_moveis > 0],
        ...
        // Eventuais
        ['nome' => 'Galo da Madrugada', 'icone' => '☕', 'desc' => 'Registrou algo de manhãzinha (5h as 9h).', 'desbloqueada' => $c_galo > 0],
